...
class DQNAgent():
    def __init__(self, state_n=4, action_n=2, hidden_n=64, learning_rate=0.001, gamma=0.99, memory_size=10000,
                 batch_size=32, target_update_interval=100, episodes=1000, policy='e-greedy', epsilon=1.0, eval_interval=100,
                 temperature=0.2, use_memory=True, use_target_network=True):
    ...
    def update(self):
        ...
        if self.use_target_network:
            ...
            # find the maximum Q-value among all possible actions in the next state, providing the best action's value according to the target network.
        else:
            next_Q_values = self.policy_model_nn(next_states).max(1)[0].unsqueeze(1)
            # find the maximum Q-value among all possible actions in the next state, providing the best action's value according to the current network.
